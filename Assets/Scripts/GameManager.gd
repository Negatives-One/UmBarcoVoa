...
func saveSettings() -> void:
	var saveSystem : SaveSystem = load("res://Saves/SaveGame.gd").new()
	saveSystem.AudioBool = audioBool
	var dir : Directory = Directory.new()
	if not dir.dir_exists("res://Saves/"):
		var _error : int = dir.make_dir_recursive("res://Saves/")
	var _error : int = ResourceSaver.save("res://Saves/Save.res", saveSystem)

func loadSettings() -> bool:
	var dir : Directory = Directory.new()
	if not dir.file_exists("res://Saves/Save.res"):
		return false
	
	var saveResource = load("res://Saves/Save.res")
